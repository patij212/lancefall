...
- De-tic "remember" to 2-3 anchors (keep the tagline + "REMEMBER EVERYTHING"). Combo-tier lines = 6 rising sentences matching all 6 `tierCombo` cut points, no countdown. Boss quotes: taunts→regrets (keep dossier prose). THE CHOICE: drop "remembers your name", new prompt/buttons — KEEP the `stillpoint.test.ts` heads ('THE LIGHT HOLDS'/'THE LIGHT RELEASED'), change only `.line`. Restore restraint to meta/player-facing lore. Verify no test asserts on edited substrings.

**WS-7 SURVIVABILITY** (`types.ts`, `tune.ts`, `perks.ts`, `game.ts`, `heat.ts`, `world.ts`)
- ⚠️ **CONFIRM INTENT FIRST** (see Phase 0). Default recommendation if unanswered: **Option A — ARMOR hit-buffer**, `defaultShields: 2`, `postHitIframe ~0.85`, reuse the LAST BREATH push loop, order **shields→LAST BREATH→revive→death**, HUD pips, +1 regen on boss-clear, a `heat.ts shieldsLost` strip column. Factor the pre-death checks into a PURE `consumeSurvivalCharge()` (like `clutch.ts`) and unit-test it. Determinism-safe (positions only, no rng).
...
## PHASE 0 — RESOLVE GATING QUESTIONS BEFORE CODE
If the owner hasn't answered, proceed on these DEFAULTS and note the assumption: **(7)** Option A ARMOR buffer, `defaultShields:2`, ON for everyone. **(8a)** HYBRID-as-procedural-stems (no asset loading). **(4)** Daily `oneLife` ON. **(6)** keep both motif anchors. **(8c)** `OfflineAudioContext`-baked one-shots allowed. Everything else needs no gate — start immediately.
